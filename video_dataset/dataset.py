...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.load_labels:
            parts = line.split(' ')
            if len(parts) == 4:  # path start_frame end_frame label
                path, start_frame, end_frame, label = parts
                start_frame, end_frame = int(start_frame), int(end_frame)
                label = int(label)
            else:  # fallback to original format: path label
                path, label = parts
                start_frame, end_frame = None, None
                label = int(label)
        else:
            parts = line.split(' ')
            path = parts[0]  # the list can be with or without labels
            if len(parts) >= 3:  # path start_frame end_frame [label]
                start_frame, end_frame = int(parts[1]), int(parts[2])
            else:
                start_frame, end_frame = None, None
            label = None
        ...
        vr = VideoReader(io.BytesIO(raw_data), num_threads=1)
        total_frames = len(vr)
        # raw_data = load_binary(path)
...
# Apply frame range if specified
        if start_frame is not None and end_frame is not None:
            ...
            if available_frames <= 0:
                raise ValueError(f"No frames available for {path} with range {start_frame}:{end_frame}")
        else:
            start_frame = 0
            available_frames = total_frames

        if self.random_sample:
            frame_idx = self._random_sample_frame_idx(available_frames)
            frame_idx = [idx + start_frame for idx in frame_idx]
            #frames = [frames[x].to_rgb().to_ndarray() for x in frame_idx]
                # Second: Extract ONLY those specific frames
            frames = vr.get_batch(frame_idx).asnumpy()
            frames = torch.as_tensor(frames).float() / 255.  
            ...
            # For non-random sampling, we need all frames in range to apply temporal crops later
            if start_frame is not None and end_frame is not None:
